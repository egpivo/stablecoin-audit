...
fn read_control_qa(out_dir: &Path) -> Result<ControlQaReport> {
...
fn read_control_provenance(out_dir: &Path) -> Result<ControlProvenanceReport> {
...
fn write_benchmark_csv(out_dir: &Path, rows: &[BenchmarkRow<'_>]) -> Result<()> {
