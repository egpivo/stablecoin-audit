...
async function apiFetch(path, options = {}) {
  if (typeof window.demoApiFetch === "function") {
    return window.demoApiFetch(path, options);
  }
...
async function loadDemoRun() {
  ...
  const preferRunId =
    window.STABLECOIN_AUDIT_DEMO?.defaultRun?.run_id || "demo_001";
  const preferAsset =
    window.STABLECOIN_AUDIT_DEMO?.defaultRun?.asset || "USDC";
  const prefer = allRuns.find(
    (r) =>
      r.run_id === preferRunId &&
      r.asset.toUpperCase() === preferAsset.toUpperCase()
  );
...
async function checkHealth() {
  const demo = window.STABLECOIN_AUDIT_DEMO?.enabled;
  const okText = (v) =>
    demo ? `Demo snapshot · toolkit ${v}` : `API ok · toolkit ${v}`;
  const badText = demo ? "Demo bundle failed to load" : "API unreachable";
  try {
...
if (window.STABLECOIN_AUDIT_DEMO?.enabled) {
