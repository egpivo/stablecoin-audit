...
Written to `out/<asset>/qa_report.json` by `transfer-audit`.
...
Each `QaChain` includes a `gates` object with the following fields:

| Gate field | Description |
|---|---|
| `metadata_calls_pass` | Metadata probes passed (`name`, `symbol`, `decimals`, live `totalSupply`) |
| `historical_total_supply_calls_pass` | Historical boundary supply calls passed (`start-1`, `end`) |
| `transfer_logs_fetched_pass` | Transfer logs fetch completed for the configured window |
| `no_duplicate_logs_pass` | No duplicates by `(chain, contract_address, tx_hash, log_index)` |
| `transfer_decode_pass` | Full Transfer decode passed for fetched logs |
| `supply_invariant_pass` | `sum(mints) - sum(burns) == totalSupply(end) - totalSupply(start-1)` |
| `provenance_stamped_pass` | Required provenance fields are stamped |
| `no_simulated_data_pass` | Data source is real on-chain RPC data |

Values are `PASS`, `FAIL`, or `UNAVAILABLE` (for unavailable checks after hard failures).

## v0.1.5 Stress Summary (experimental report)

Generated by `report` from existing v0.1 artifacts (`supply_audit.csv`, `qa_report.json`, `provenance.json`).

Outputs:
- `out/<asset>/v0_1_5_stress_summary.csv`
- `out/<asset>/v0_1_5_summary.md`

Scope:
- accounting stress summary (e.g., invariant/QA anomalies, mint/burn spikes)
- activity stress summary (e.g., transfer/active-address spikes)

Non-scope:
- purchasing power, peg, reserve adequacy, dilution detection
- control-event scoring, CCTP matching, bridged backing, market/oracle pricing
